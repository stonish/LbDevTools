...
def rel_project_path(path, to):
    ...
    path = path.split('/')
    to = to.split('/')
    parts = list(itertools.dropwhile(
        lambda x: x[0] < len(to) and to[x[0]] == x[1],
        enumerate(path)))
...
def find_merge_request_issues(mr, project_fullname):
    ...
    jira_tasks = (
        re.findall(jira_re, mr.title) + re.findall(jira_re, mr.description))
    gl_re = r'(?:((?:[^/\s]+/)*)([^/\s]+))?(#[0-9]+)'
    gl_issues = [norm(*m) for m in
                 re.findall(gl_re, mr.title) +
                 re.findall(gl_re, mr.description)]
...
def find_merge_requests(server, repo, since, milestone):
    ...
    if not gitlab_mrs:
        logging.warning('No merge requests found with {} milestone for {}'
                        .format(milestone, project_fullname))
    else:
        ...
        for mr in gitlab_mrs:
            if mr.id not in [x.id for x in git_mrs]:
                logging.warning('MR {} is not merged in current branch '
                                'after tag {} (state {})'
                                .format(mr.web_url, since, mr.state))
                git_mrs.append(mr)

    for mr in git_mrs:
        mr.issue_refs = find_merge_request_issues(
            mr, project_fullname)
...
def find_dependencies(stack_config, project_name):
    ...
    while remaining:
        dependencies = set().union(
            *(stack_config.get(name, (None, None, []))[2]
...
def get_template(template, template_paths):
    ...
    def select_mrs(mrs, labels, used=None):
        ...
        '''
        label_sets = [set([ls] if isinstance(ls, six.string_types) else ls)
                      for ls in labels]
...
''',
    ...
    def get_dep_mrs():
        ...
        for name, (previous, target, _) in dependencies.items():
            if target and previous and target != previous:
                dep_mrs.extend(find_merge_requests(
                    server,
    ...
    for name, (_, ver, _) in dependencies.items():
        if name == 'LCG':
            ...
            project_deps.append(
                'LCG [{ver}](http://lcginfo.cern.ch/release/{ver}/) {text}'
                .format(ver=ver, text=text))
        else:
            repo = os.path.join(os.path.dirname(args.stack), name)
            relpath = rel_project_path(guess_project_fullname(repo),
                project_fullname)
            project_deps.append('{name} [{ver}]({dots}/{relpath}/-/tags/{ver})'
                .format(
                    name=name, ver=ver,
