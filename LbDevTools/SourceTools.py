...
COPYRIGHT_STATEMENT = '''
(c) Copyright {year} CERN for the benefit of the LHCb Collaboration
...
def add_copyright_to_file(path, year=None, licence_file='COPYING'):
    '''
    ...
    text = to_comment(
        COPYRIGHT_STATEMENT.format(
            year=year or date.today().year, licence=licence_file).strip(),
        lang)
    with open(path, 'rb') as f:
        data = f.readlines()
...
def add_copyright():
    ...
    parser.add_argument(
        '--licence-file',
        help='name of the licence file (default: %(default)s)',
        default='COPYING')
    parser.add_argument(
        '--force',
        action='store_true',
    ...
    for path in args.files:
        if not args.force and not to_check(path):
            ...
            add_copyright_to_file(path, args.year, args.licence_file)
