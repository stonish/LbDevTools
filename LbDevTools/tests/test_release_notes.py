...
def test_script():
    ...
    try:
        ...
        # check preamble
        assert ('{} {} {}'.format(datetime.date.today(), project,
                                  STACK[project][1]) in lines[0])
        assert re.search(r'LHCb.*{}'.format(STACK['LHCb'][1]), output)
        assert re.search(
            r'relative to {}.*{}'.format(project, STACK[project][0]), output)
        # check for changes
        assert (('~Calo ~"MC checking" | Allow relations changes in '
                 'LHCb, !385 (@graven)') in output)
        # check for highlights from upstream projects
        assert (('~Decoding ~Muon | Improve MuonRawToHits, '
                 'LHCb!2177 (@rvazquez)') in output)
    finally:
        os.chdir(old_path)
        shutil.rmtree(path)
